...
| DX13 | 参数 LSP 完整支持 | ✅ | KL-01 参数引用含声明位置 + KL-02 参数重命名 + 声明位置定义精确化 + 签名悬停。DX13-01~09（16 asserts）。讨论 → [D_LspUsabilityAudit](Discussion/D_LspUsabilityAudit.md) | ⭐⭐ |
...
**Lang 系列全部完成。DX13 ✅ 完成。DX14~DX16 ⏳ 待排期（LSP 审查改进）。当前位置 → DX14。**
