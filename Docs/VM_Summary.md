...
| DX17 | 统一符号解析 | ⏳ | 合并 SymbolAtPosition + FindDefinitionLocation → ResolvedSymbol。HandleDefinition/HandleReferences/HandleRename/HandleHover 共享 ResolveSymbol。消除 ResolveSymbolDualAst 二次查找。修复 WaitForStmt 子表达式遗漏。讨论 → [D_LspStructuralAudit](Discussion/D_LspStructuralAudit.md) | ⭐⭐ |
...
| LSP 架构 | DX12 后台编译（远期）、DX13~DX16 审查改进 ✅、DX17~DX18 结构性重构（⏳） | DX10 ✅、DX11 ✅ |
