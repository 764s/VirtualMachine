...
| **Lang-15** | **Include 可见性（public/private）** | **⏳** | **`private` / `public` 可见性修饰符用于 include mixin 名称隔离。private 声明仅源文件内可见，不参与跨文件命名冲突；public 声明可被 includer 引用（现有行为）。Phase A（本期）：默认 public（向后兼容零 breaking），新增 `private` / `public` 关键字，Preprocessor origin-aware 合并，编译器 origin-aware 符号查找。与 @export 完全正交（两轴隔离，4 种组合均合法）。可行性分析 → [D_PublicPrivateVisibility.md](Discussion/D_PublicPrivateVisibility.md)。步骤计划 → [Step_Lang15](Plan/Step_Lang15_PublicPrivateVisibility.md)** | **include 冲突减少 + 模块封装** | **编译器（Lexer + Parser + Preprocessor + BytecodeCompiler）+ LSP** | **⭐⭐⭐** |

> Phase 1（Lang-1~Lang-3）✅ 完成。Phase 2 = Lang-6~Lang-8（Q4 服务脚本 XCALL 路径），Lang-6 ✅ 完成，Lang-7 ✅ 完成，Lang-8 ✅ 完成。Phase 2 全部完成。Lang-4 按需触发（黑板瓶颈时）。**Lang-9 A5 深度内联 P1 ✅ P2 ✅ P3 ✅ P4 ✅ 全部完成（P1 模块内 trivial inline：IN01-IN08。P2 模块内一般内联：多语句/分支/循环/多 return exit label/用户函数嵌套/struct 参数/void/cleanup 守护/FO6 窗口修正/tempBaseline，IN09-IN23。P3 跨模块内联：ModuleInlineInfo + TryInlineMemberCall + _xInlineVars 导出变量重定向 XLOAD_MVAR/XSTORE_MVAR，XIN01-XIN10。P4 深度链式内联：callee 内部函数递归内联 + TryInlineCalleeFunc + visited 防环，DIN01-DIN05，1517 测试总计）。** Lang-10 ✅ 完成（导出变量默认值 + 批量提取 API，SK15 需求驱动）。Lang-11 ✅ 完成（模块级 struct var/const 直接初始化，MSV01-MSV15 全通过）。Lang-12 ✅ 完成（@export const 基础类型，EC01-EC03 全通过）。**Lang-13 ✅ 完成**（语法糖枚举，EN01-EN12 + LSP-EN01~EN05，1627 测试总计）。**Lang-14 ✅ 位运算支持完成**。当前位置 → **Lang-15 ⏳ Include 可见性（public/private）**。 → [Step_Lang9](Plan/Step_Lang9_DeepInlining.md), [Step_Lang13](Plan/Step_Lang13_Enum.md), [Step_Lang14](Plan/Step_Lang14_Bitwise.md), [Step_Lang15](Plan/Step_Lang15_PublicPrivateVisibility.md)
>
...
> - [x] 全部 1675 测试通过（1627 原有 + 48 新增位运算测试），B01-B06 无回归
>
