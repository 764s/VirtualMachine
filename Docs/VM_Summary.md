...
| 测试 | 1411 项 Assert 全部通过（112 TreeWalker + 910 Compiler + 44 Performance + 18 FFScript + 51 Debug + 97 DAP + 179 LSP），另有 6 项自动化性能基准 | ✅ Lang-9 P1 通过 |
...
| *Lang-9* | *A5 深度内联* | *⏳ P1 ✅* | *编译器对所有满足条件的函数主动内联（无论是否标注 `@inline`）。`@inline` 不改变内联行为，仅在标注函数无法内联时触发诊断（Warn/Error，由 `InlineFailurePolicy` 配置控制）。未标注函数静默退化。P1 模块内 trivial inline ✅（单 return 纯表达式函数，CanInline + EstimateInstructionCount + TryInlineCall + @inline 诊断，IN01-IN08 全通过）。P2 模块内一般内联 ⬚。P3 跨模块内联 ⬚。P4 深度链式内联 ⬚。可行性分析 → [D_DeepInlining.md](Discussion/D_DeepInlining.md)。步骤计划 → [Step_Lang9](Plan/Step_Lang9_DeepInlining.md)* | *极致优化* | *编译器* | *⭐⭐⭐* |
