...
| 测试 | 1388 项 Assert 全部通过（112 TreeWalker + 887 Compiler + 44 Performance + 18 FFScript + 51 Debug + 97 DAP + 179 LSP），另有 6 项自动化性能基准 | ✅ Lang-12 通过 |
...
| *Lang-9* | *A5 深度内联* | *⏳* | *编译器对所有满足条件的函数主动内联（无论是否标注 `@inline`）。`@inline` 不改变内联行为，仅在标注函数无法内联时触发诊断（Warn/Error，由 `InlineFailurePolicy` 配置控制）。未标注函数静默退化。O4 跨模块常量折叠。编译期完整语义保持。可行性分析 → [D_DeepInlining.md](Discussion/D_DeepInlining.md)* | *极致优化* | *编译器* | *⭐⭐⭐* |
...
> Phase 1（Lang-1~Lang-3）✅ 完成。Phase 2 = Lang-6~Lang-8（Q4 服务脚本 XCALL 路径），Lang-6 ✅ 完成，Lang-7 ✅ 完成，Lang-8 ✅ 完成。Phase 2 全部完成。Lang-4 按需触发（黑板瓶颈时）。Lang-9 A5 深度内联（编译器主动内联所有可内联函数，`@inline` 仅控制诊断，可行性分析 → [D_DeepInlining.md](Discussion/D_DeepInlining.md)）。**Lang-10 ✅ 完成（导出变量默认值 + 批量提取 API，SK15 需求驱动）。Lang-11 ✅ 完成（模块级 struct var/const 直接初始化，MSV01-MSV15 全通过）。Lang-12 ✅ 完成（@export const 基础类型：ProcessModuleVariables 为导出 const 分配寄存器 + BuildExportTable 生成 Writable=false 条目 + 运行时 XSTORE_MVAR Writable 防御检查，EC01-EC03 全通过，1388 测试总计）。需求方可启动 Phase 2 宿主侧重构。** → [Step_Lang11](Plan/Step_Lang11_ModuleStructInit.md)
>
