...
| *DX4-P2* | *CLI 项目编译集成* | *⏳* | *`ffvm-cli init` 脚手架生成 `.ffproj` 模板（带注释，支持 `--host` 预设） + `ffvm-cli compile --project x.ffproj` 读取 `.ffproj` 构建编译参数。LSP 和 CLI 使用相同的项目配置。讨论来源 → [D_Usability_ModuleScope.md](Discussion/D_Usability_ModuleScope.md) §6.5* | *CLI 生产编译 + 项目脚手架* | *CLI* | *⭐⭐* |
