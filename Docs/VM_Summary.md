...
| 测试 | 1492 项 Assert 全部通过（112 TreeWalker + 991 Compiler + 44 Performance + 18 FFScript + 51 Debug + 97 DAP + 179 LSP），另有 6 项自动化性能基准 | ✅ Lang-9 P3 通过 |
...
> Phase 1（Lang-1~Lang-3）✅ 完成。Phase 2 = Lang-6~Lang-8（Q4 服务脚本 XCALL 路径），Lang-6 ✅ 完成，Lang-7 ✅ 完成，Lang-8 ✅ 完成。Phase 2 全部完成。Lang-4 按需触发（黑板瓶颈时）。**Lang-9 A5 深度内联 P1 ✅ P2 ✅ P3 ✅ 完成（P1 模块内 trivial inline：IN01-IN08。P2 模块内一般内联：多语句/分支/循环/多 return exit label/用户函数嵌套/struct 参数/void/cleanup 守护/FO6 窗口修正/tempBaseline，IN09-IN23。P3 跨模块内联：ModuleInlineInfo + TryInlineMemberCall + _xInlineVars 导出变量重定向 XLOAD_MVAR/XSTORE_MVAR，XIN01-XIN10，1492 测试总计）。P4 后续。** Lang-10 ✅ 完成（导出变量默认值 + 批量提取 API，SK15 需求驱动）。Lang-11 ✅ 完成（模块级 struct var/const 直接初始化，MSV01-MSV15 全通过）。Lang-12 ✅ 完成（@export const 基础类型，EC01-EC03 全通过）。 → [Step_Lang9](Plan/Step_Lang9_DeepInlining.md)
>
