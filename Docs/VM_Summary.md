...
| *DX4-P1* | *`.ffproj` 项目描述文件* | *⏳* | *JSON 格式定义与解析 + LSP 启动时发现并加载 `.ffproj` + `includePaths` → IFileResolver 搜索路径 + `hostDeclarations` → 自动加载 `.ffvm.d.json` + `entry` / `compileOptions` 字段。效果：完整模块组合编译 + 诊断。讨论来源 → [D_Usability_ModuleScope.md](Discussion/D_Usability_ModuleScope.md)* | *项目级编译配置* | *LSP + 编译器* | *⭐⭐⭐* |
