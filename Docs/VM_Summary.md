...
- 固化讨论文件
  // 之后的讨论都会通过修改讨论文件达成
  // 讨论完成前, 文件末尾有一个专用的讨论区用于交流
- 向串行需求列表提交需求
- 未串行需求列表规划 checklist 子计划文件
- 推进子计划至完成

### 额外需求
优化 `#check-and-next`, 整合当前实现流程和老指令中的执行细节, 确保实施这个指令时能严格按顺序执行
...
Docs/
  ...
  Discussion/                ← 讨论区：设计探讨 / 方案对比 / 决策记录
    ...
    D_PublicPrivateVisibility.md     Include 可见性 public/private 设计讨论（✅ 已完成 → Lang-15）
    D_Usability_ModuleScope.md       易用性补充：语法信息来源范围 × 编译方式（✅ 已完成 → DX4~DX5 系列）
    D_LspArchitecture.md             LSP 架构演进：全项目诊断、状态管理与增量更新（💬 讨论中 → DX10~DX12）
  Plan/                      ← 计划区：确定步骤 / 无脑执行
    ...
    Step_Lang1_1a_MaxRegisters_Config.md  Lang-1.1a MaxRegisters 常量配置化
    Step_Lang1_1b_ExtendedRegisters.md    Lang-1.1b 扩展寄存器
    Step_Lang2_Include.md                 Lang-2 include 预处理器
    Step_Lang3_Blackboard_Syscall.md      Lang-3 黑板 Syscall 正式化
    Step_Lang15_PublicPrivateVisibility.md  Lang-15 Include 可见性（public/private）
...
| 测试 | 1675 项 Assert 全部通过（114 TreeWalker + 1153 Compiler + 44 Performance + 18 FFScript + 51 Debug + 97 DAP + 198 LSP），另有 6 项自动化性能基准 | ✅ Lang-14 通过 |
...
| 优先级 | 内容 | 阻塞关系 |
|--------|------|----------|
| **P0** | **语言需求实现（SK14）**— 模块变量 (L1) + include (L2) + 黑板 Syscall 正式化 (L3)；远期：跨模块共享变量 (L4) + 跨模块函数调用 (L5)。详见 §7 Lang 区。 | **✅ Phase 1 完成**。Lang-1~Lang-3 全部完成 |
...
**B 阶段全部完成。1007 项 Assert × 2 模式全通过。B-ε 性能优化串行计划全部完成（4/4）。B-ζ 分支场景优化串行计划全部完成（3/3）。B-η O8 指令压缩完成（O8-1~O8-3/O8-5 ✅，O8-4 ⏸）。D 阶段分发基础设施全部完成（DIST-1~DIST-10 ✅）。Lang-1 ✅ 完成。Lang-1.1a ✅ 完成。Lang-1.1b ✅ 完成。Lang-2 ✅ 完成。Lang-3 ✅ 完成。Lang-6 ✅ 完成（benchmark 无回归确认）。Lang-7 ✅ 完成（自动退化 + VMConfig）。Lang-8 ✅ 完成（统一语法 + @inline + LSP 警告诊断）。Lang-9 P1 ✅ 完成（模块内 trivial inline，IN01-IN08）。Lang-9 P2 ✅ 完成（模块内一般内联，IN09-IN23）。Lang-9 P3 ✅ 完成（跨模块内联，XIN01-XIN10）。Lang-9 P4 ✅ 完成（深度链式内联，DIN01-DIN05）。Lang-11 ✅ 完成（模块级 struct var/const 直接初始化，MSV01-MSV15）。Lang-12 ✅ 完成（@export const 基础类型，EC01-EC03）。**
...
**B-ζ 分支场景优化串行计划完成（3/3 ✅）。B-η O8 指令压缩完成（O8-1~O8-3/O8-5 ✅，O8-4 临时妥协 ⏸）。D 阶段分发基础设施全部完成（DIST-1~DIST-10 ✅）。Lang-1 ✅ 完成。Lang-1.1a ✅ 完成。Lang-1.1b ✅ 完成。Lang-2 ✅ 完成。Lang-3 ✅ 完成。**
...
| *Lang-9* | *A5 深度内联* | *✅ P1 ✅ P2 ✅ P3 ✅ P4 ✅* | *编译器对所有满足条件的函数主动内联（无论是否标注 `@inline`）。`@inline` 不改变内联行为，仅在标注函数无法内联时触发诊断（Warn/Error，由 `InlineFailurePolicy` 配置控制）。未标注函数静默退化。P1 模块内 trivial inline ✅（单 return 纯表达式函数，IN01-IN08）。P2 模块内一般内联 ✅（多语句、分支、循环、多 return exit label、用户函数嵌套内联、struct 参数、void 内联、cleanup 块守护、FO6 窗口修正、临时寄存器 baseline，IN09-IN23）。P3 跨模块内联 ✅（ModuleInlineInfo 传递 callee AST + VarExportIndices + ConstValues，TryInlineMemberCall 在 XCALL 前内联展开，_xInlineVars 重定向 callee 导出变量到 XLOAD_MVAR/XSTORE_MVAR，XIN01-XIN10）。P4 深度链式内联 ✅（callee 内部函数递归内联：BuildModuleInlineInfo 包含全部函数 AST，CanInlineCrossModule visited 集合防无限递归，TryInlineCalleeFunc 在跨模块内联体内递归展开 callee 自身函数调用，CompileExprStmt/CompileExpr 拦截 _xInlineInfo 上下文中的 callee 函数调用，DIN01-DIN05）。可行性分析 → [D_DeepInlining.md](Discussion/D_DeepInlining.md)。步骤计划 → [Step_Lang9](Plan/Step_Lang9_DeepInlining.md)* | *极致优化* | *编译器* | *⭐⭐⭐* |
...
| *Lang-11* | *模块级 struct const 直接初始化 (方案 A)* | *✅* | *支持模块级 `var`/`const` 声明 struct 类型并以 struct literal 初始化（如 `const hitPhase: HitPhaseDef = HitPhaseDef { ... }`）。方案 A：编译器在 `ProcessModuleVariables` 阶段识别 struct literal 初始化器，为每个字段分配连续模块变量寄存器，`TryFoldStructLiteral` 递归折叠字段值，`EmitModuleVarInit` 展开为逐字段 LOAD_CONST+STORE_MVAR。const 防写 + 嵌套 struct + 函数参数传递。纯编译期特性，无运行时改动。MSV01-MSV15 (55 asserts) 全通过* | KOF collision 配置提升至模块级 | 编译器 | ⭐⭐ |
| **Lang-12** | **@export const（基础类型）** | **✅** | **支持 `@export const name: int = expr` / `@export const name: float = expr` 导出模块级常量（基础类型 int/float）。编译器在 `ProcessModuleVariables` 阶段为 `@export const` 分配模块变量寄存器（供 ExportTable mvarSlot），同时保留 `_moduleConstValues` 编译时折叠。`BuildExportTable` 生成 `ExportVarEntry(Writable=false, DefaultValue=折叠值)`。宿主通过 `GetVarDefault()` / `XLOAD_MVAR` 读取。赋值编译期拒绝。运行时 XSTORE_MVAR Writable 检查（防御性）。Preprocessor 已支持同类型跨文件覆盖（const 覆盖 const）。本期仅基础类型，struct 类型 @export const 后续扩展。EC01-EC03 全通过** | **KOF 技能配置 ROM 化（`@export var` → `@export const`）** | **编译器 + ExportTable** | **⭐⭐** |
| **Lang-13** | **语法糖枚举 (enum)** | **✅** | **`enum Name { A, B = expr, C }` 语法糖 → 编译期展开为 `const Name.A: int = 0` / `const Name.B: int = expr` / `const Name.C: int = prev+1` 一组命名整数常量。纯编译期特性（零运行时改动、零新 OpCode）。Lexer 新增 `enum` 关键字。AST 新增 `EnumDecl` 节点 + `ModuleNode.Enums` 列表。Parser 解析花括号成员列表（自动递增 / 显式赋值）。编译器 `ProcessEnums` 在 `ProcessModuleVariables` 前展开到 `_constValues`（`EnumName.Member` 键名）。`FieldAccessExpr` 编译路径拦截枚举成员引用（编译期常量折叠 → LOAD_CONST）。LSP 支持：枚举染色（TextMate `enum` keyword）、补全（`EnumName.` 点号 → 成员列表）、hover、documentSymbol、definition、references。步骤计划 → [Step_Lang13_Enum.md](Plan/Step_Lang13_Enum.md)** | **语言易用性：整数常量组命名** | **编译器 + LSP** | **⭐⭐** |
| **Lang-14** | **位运算支持** | **✅** | **6 个位运算符：`&`（按位与）、`|`（按位或）、`^`（按位异或）、`~`（按位取反）、`<<`（左移）、`>>`（右移）。Lexer 6 新 TokenType（`Amp`/`Pipe`/`Caret`/`Tilde`/`LtLt`/`GtGt`），`&`/`|` 从 Error 回退改为合法单字符 token，`<`/`>` 扩展为 3 路判定（`<=`/`<<`/`<`、`>=`/`>>`/`>`）。AST 6 新 NodeKind（`BitAnd`/`BitOr`/`BitXor`/`BitNot`/`Shl`/`Shr`）。Parser 插入 4 个新优先级层（`\|` < `^` < `&`，位于 LogicalAnd 和 Equality 之间；`<<`/`>>` 位于 Comparison 和 Addition 之间；`~` 一元运算与 `!`/`-` 同级）。Number 新增位运算静态方法（Fix64 + dev 双模式，整数域操作）。编译器 6 新 OpCode（BIT_AND/BIT_OR/BIT_XOR/BIT_NOT/SHL/SHR）+ BinOpCode/UnOpCode 映射 + TryFoldConstant 常量折叠 + GetRegisterMask 注册 + IsResultProducer Peephole 适配。VM 解释循环 6 新 case。TreeWalker 6 新 case。内联自动支持（BinaryExpr/UnaryExpr 已在 IsInlineSafeExpr 中处理）。步骤计划 → [Step_Lang14_Bitwise.md](Plan/Step_Lang14_Bitwise.md)** | **技能 tag/flag 位操作（`1 << N`、`tags \| flag`、`tags & mask`）** | **全链路（Lexer→Parser→Compiler→VM→TreeWalker）** | **⭐⭐** |
| **Lang-15** | **Include 可见性（public/private）** | **✅** | **`private` / `public` 可见性修饰符用于 include mixin 名称隔离。Phase A（默认 public，向后兼容零 breaking）。Lexer 新增 `Public`/`Private` TokenType + 2 关键字。AST: `FuncDecl.IsPrivate`/`OriginFile`、`VarDeclStmt.IsPrivate`/`OriginFile`、`StructDecl.IsPrivate`/`OriginFile`、`EnumDecl.IsPrivate`/`OriginFile`。Parser: 支持 `private`/`public` 前缀（+ @export/@inline 自由组合）。Preprocessor: origin-aware 合并（private 声明用 `name\0originFile` qualified key，跨文件不冲突）。BytecodeCompiler: `FuncKey`/`ResolveFuncKey`/`ResolveStructKey`/`InjectPrivateModuleSymbols`。内联适配：inline 展开时切换 `_currentOriginFile`。与 @export 完全正交（4 种组合合法）。PV01-PV18 测试 + TW 兼容。TextMate `storage.modifier.ffvm`。20 个关键字，1704 测试总计。可行性分析 → [D_PublicPrivateVisibility.md](Discussion/D_PublicPrivateVisibility.md)。步骤计划 → [Step_Lang15](Plan/Step_Lang15_PublicPrivateVisibility.md)** | **include 冲突减少 + 模块封装** | **编译器（Lexer + Parser + Preprocessor + BytecodeCompiler）+ LSP** | **⭐⭐⭐** |
| **Lang-16** | **Override 关键字** | **✅** | **`override` 修饰符用于跨文件声明替换的显式化。Lexer 新增 `Override` TokenType + `"override"` 关键字（21 个关键字）。AST: `FuncDecl.IsOverride`/`VarDeclStmt.IsOverride`/`StructDecl.IsOverride`/`EnumDecl.IsOverride`。Parser: `override` 前缀可与 `public`/`@export`/`@inline` 组合，但不可与 `private` 组合。Preprocessor: 跨文件替换无 `override` → 报错；`override` 指向不存在的声明 → 报错；diamond include 同源 → 静默跳过。include chain 中子文件亦需显式 `override`。OV01-OV16 测试，1734 测试总计（114 TW + 1212 Compiler + 44 Perf + 18 FFS + 51 Debug + 97 DAP + 198 LSP）** | **include 替换安全性** | **编译器（Lexer + Parser + Preprocessor）** | **⭐⭐** |
| **Lang-17** | **Include As 别名（命名空间隔离）** | **✅** | **`include "path" as Alias` 将被包含文件的 public 声明置于 `Alias.Name` 命名空间下，不执行平坦 mixin 合并。支持 `Alias.func()`/`Alias.Struct`/`Alias.Enum.Member`/`Alias.var`/`Alias.const` 访问。`as` 为上下文关键字（仅 include 语句中生效）。向后兼容：无 `as` 的 include 保持原有 mixin 行为。可行性分析 → [D_IncludeAs.md](Discussion/D_IncludeAs.md)。步骤计划 → [Step_Lang17](Plan/Step_Lang17_IncludeAs.md)** | **多模块同名声明共存** | **编译器（Parser + Preprocessor + BytecodeCompiler）** | **⭐⭐⭐** |
| *Lang-18* | *Override Alias 声明* | **✅** | **`override func Alias.Name()` / `override const Alias.Name` / `override struct Alias.Name` / `override enum Alias.Name` 替换别名模块声明。AST: 4 个声明节点新增 `AliasTarget` 属性。Parser: 点号名称语法（仅 override 上下文）。Preprocessor: `ApplyAliased{Func|Var|Struct|Enum}Override` 在别名模块中替换对应声明。BytecodeCompiler: 零改动。前置：Lang-17 ✅。步骤计划 → [Step_Lang18](Plan/Step_Lang18_OverrideAlias.md)** | **可组合框架模式（引入模板模块 + 定制部分行为）** | **编译器（Parser + Preprocessor）** | **⭐⭐** |
| *DX4-P0* | *LSP workspace 快速改善* | *✅* | *LSP rootUri → 磁盘 FileSystemFileResolver 构建 + AllDirectories 自动发现 workspace 内 `.ffvm.d.json` 加载 + entryFunc=null 跳过入口检查。效果：单 workspace 内 include 路径解析 + syscall 补全基本可用。FileSystemFileResolver 从 KOF98 迁移至 FFVM 核心库。DX4-P0-01~13 全通过，1791 测试总计。讨论来源 → [D_Usability_ModuleScope.md](Discussion/D_Usability_ModuleScope.md)* | *LSP 多文件易用性* | *LSP Server + 编译器* | *⭐⭐* |
| *DX4-P1* | *`.ffproj` 项目描述文件* | *✅* | *JSON 格式项目描述文件：`ProjectFile` 解析类 + `CompositeFileResolver` 多路径 include 搜索 + LSP 启动时自动发现并加载 `.ffproj`。字段：`includePaths` → 多目录 IFileResolver 搜索路径 + `hostDeclarations` → 指定 `.ffvm.d.json` 加载 + `entry` / `compileOptions`（inlineThreshold/inlineDepthMax/maxHoistedPerLoop/diagnosticsEnabled）。DX4-P1-01~16 全通过，1834 测试总计。讨论来源 → [D_Usability_ModuleScope.md](Discussion/D_Usability_ModuleScope.md)* | *项目级编译配置* | *LSP + 编译器* | *⭐⭐⭐* |
| *DX4-P2* | *CLI 项目编译集成* | *✅* | *`ffvm-cli init` 脚手架生成 `.ffproj` 模板（支持 `--host` 预设） + `ffvm-cli compile --project x.ffproj` / `ffvm-cli run --project x.ffproj` 读取 `.ffproj` 构建编译参数（FileResolver + hostDeclarations syscall 合并 + CompileOptions 转发）。共享逻辑 ProjectFile.GenerateTemplate / LoadHostDeclarations / MergeDeclarationJson。DX4-P2-01~13 全通过，1873 测试总计。讨论来源 → [D_Usability_ModuleScope.md](Discussion/D_Usability_ModuleScope.md) §6.5* | *CLI 生产编译 + 项目脚手架* | *CLI* | *⭐⭐* |
| *DX4-P3* | *跨文件符号查询* | *✅* | *合并 AST 缓存（`_mergedAsts` via Preprocessor）+ `FilePathToUri`（OriginFile→URI）+ `ResolveOriginUri`（跨文件/同文件判定）+ HandleDefinition/HandleReferences/HandleHover/HandleCompletion/HandleSignatureHelp 全部使用合并 AST。DX4-P3-01~13 全通过，1901 测试总计。讨论来源 → [D_Usability_ModuleScope.md](Discussion/D_Usability_ModuleScope.md)* | *跨文件 LSP 导航* | *LSP Server* | *⭐⭐⭐* |
| *DX4-P4* | *LSP 辅助创建 `.ffproj`* | *✅* | *`initialized` 通知触发检查（有 `.ffs` 无 `.ffproj`）→ `window/showMessageRequest`（Create/Ignore/Don't ask again）→ 用户选 Create → `workspace/applyEdit`（CreateFile + TextDocumentEdit 插入 GenerateTemplate 模板）。服务端请求基础设施（SendRequest + _pendingServerRequests + HandleServerResponse）。DX4-P4-01~13 全通过，1915 测试总计。讨论来源 → [D_Usability_ModuleScope.md](Discussion/D_Usability_ModuleScope.md) §6.5 Layer 2* | *零配置体验提升* | *LSP Server* | *⭐⭐* |
| *DX5* | *LSP 重命名 + 语义染色 + Include 导航 + 成员符号* | *✅* | *`textDocument/rename` + `prepareRename` 函数/变量/结构体/枚举重命名。`textDocument/semanticTokens/full` 结构体/枚举/字段/成员声明处语义染色。Include 路径 go-to-definition（`ResolveIncludeFileUri`）+ find-references。StructField/EnumMember 新增 `SymbolKindTag` + definition/references。`.ffproj` 自动创建使用文件夹名。`GenerateTemplate` 带注释模板 + `StripLineComments` JSON 注释处理。TextMate grammar 增强（struct/enum/type-annotation 正则染色）。DX5-01~20（38 asserts）全通过，1953 测试总计* | *LSP 编辑体验* | *LSP Server + TextMate* | *⭐⭐⭐* |
| *DX6* | *Include 文件重命名自动更新引用* | *✅* | *LSP `workspace/willRenameFiles` 请求处理：用户在文件管理器中重命名 `.ffs` 文件时，自动扫描 workspace 内所有 `include "old_path"` → 批量更新为 `include "new_path"`。覆盖：普通 include + `include as` 别名 + 跨 includePaths 搜索路径解析 + 子目录路径 + 显式 .ffs 扩展名。capability 注册 `workspace.fileOperations.willRename`。VSCode 扩展 middleware 注册。DX6-01~10（22 asserts）全通过，1975 测试总计* | *文件重命名一致性* | *LSP Server* | *⭐⭐* |
| *DX7* | *AST 精确位置追踪（字段访问 + 类型注解）* | *✅* | *R2: `FieldAccessExpr` 新增 `FieldNameLine`/`FieldNameColumn`（`.fieldName` token 精确位置）→ rename/references 覆盖 `v.x` 使用处。FindSymbolInExpr 支持字段访问位置匹配。FindDefinitionLocation/CollectReferencesWithOrigin 支持 parentName=null 全局搜索。R3: `VarDeclStmt`/`ParamDecl`/`StructField` 新增 `TypeNameLine`/`TypeNameColumn` → semantic tokens 覆盖类型注解（struct/enum 用色）。CollectTypeUsageTokens 递归遍历 block。纯 additive AST + Parser 改动，BytecodeCompiler 零变更。DX7-01~08（10 asserts）全通过，1984 测试总计* | *LSP 符号精确度* | *AST + Parser + LSP* | *⭐⭐* |
| *DX8* | *external func 语法 + 跨文件错误 + 表达式语义染色* | *✅* | *`external func Name(params): RetType` 无体声明。Parser ParseExternalFuncDecl + AST IsExternal + Compiler ExternalFuncSlotBase=90000 + 跨文件错误 AddError/IsCrossFileError + 表达式语义染色（enum.MEMBER + field access + struct literal）。DX8-01~08 全通过，2071 测试总计* | *外部函数 + 诊断改善* | *全链路* | *⭐⭐⭐* |
| *DX9* | *语义染色改进（声明 + 引用 + 类型）* | *✅* | *Legend: type/struct/enum/enumMember/property/variable/function/parameter/string/keyword。external→keyword, param→parameter, var/const→variable, IdentifierExpr→variable（排除 struct/enum/func 名）。DX9-01~08 全通过，2104 测试总计* | *编辑器着色体验* | *LSP Server* | *⭐⭐* |
| *R1* | *LSP 架构重构（AstWalker + 双 AST 统一 + DocumentStore）* | *✅* | *AstWalker 基类统一 13 组 Block/Stmt/Expr 遍历三元组（WalkBlock/WalkStmt/WalkExpr + `_abort` 早退支持）→ 13 个 walker 子类。ResolveSymbolDualAst() 统一 4 个 handler 的双 AST 符号解析（per-file→merged fallback）。DocumentStore 内部类封装 `_documents`/`_documentAsts`/`_mergedAsts` 三字典（16 处直接访问替换）。4100→3780 行（−320 行，−8%），纯重构无功能变更，2113 测试全通过* | *内部代码质量 / DX10 前置* | *LSP Server* | *⭐⭐* |
| *E004* | *模块级符号导航（go-to-definition / find-references / hover / rename）* | *✅* | *FindSymbolAtPosition 扫描 ModuleVariables（初始化器 + 类型注解 + 变量名）。FindSymbolInExpr 支持 null func 模块级上下文。FindDefinitionLocation 新增模块变量定义 + 函数作用域 fallback。CollectReferencesWithOrigin 扫描模块变量初始化器中的 func/enum/enumMember 引用。CollectEnumIdentRefsInBlock/Stmt/Expr 收集 FieldAccessExpr 中枚举类型引用。E004-01~10（17 asserts）全通过，2130 测试总计（504 LSP）* | *模块变量 LSP 完整支持* | *LSP Server* | *⭐⭐* |
| **DX10** | **依赖图 + 全项目诊断** | **✅** | Include 依赖图（`_includeDependents`/`_includeForward` 双向图 + `Preprocessor.ResolvedFilePaths` 传递性依赖追踪）。`OverlayFileResolver` 使级联重编译可见打开文档的内存内容。`RecompileDependents()` 按依赖图传播 + visited 防环 + snapshot 安全。didOpen/didChange 触发级联。`workspace/didChangeWatchedFiles` 处理外部磁盘变更。DX10-01~08 全通过（14 asserts），2127 测试总计（501 LSP）。讨论来源 → [D_LspArchitecture.md](Discussion/D_LspArchitecture.md) | *全项目错误可见性* | *LSP Server* | *⭐⭐⭐* |
| **DX11** | **VFS + Rename 状态更新** | **✅** | DocumentStore.RenameUri() URI key 迁移 + ApplyTextEdits() 内存文本编辑 + ApplyRenameState() willRenameFiles 后状态同步（text edits 预应用 + URI 迁移 + 重编译）→ 修复连续重命名失败。DX11-01~05 全通过（13 asserts），2140 测试总计（514 LSP）。讨论来源 → [D_LspArchitecture.md](Discussion/D_LspArchitecture.md)，子计划 → [Step_DX11](Plan/Step_DX11_VFS_RenameState.md) | *rename 一致性* | *LSP Server* | *⭐⭐* |
| **DX12** | **后台编译调度** | **⚪** | debounce + 取消机制 + 编译结果缓存 → 全项目诊断不阻塞编辑。开始时机：DX11 ✅ 完成后。讨论来源 → [D_LspArchitecture.md](Discussion/D_LspArchitecture.md) | *编辑体验流畅度* | *LSP Server* | *⭐⭐* |

> Phase 1（Lang-1~Lang-3）✅ 完成。Phase 2 = Lang-6~Lang-8（Q4 服务脚本 XCALL 路径），Lang-6 ✅ 完成，Lang-7 ✅ 完成，Lang-8 ✅ 完成。Phase 2 全部完成。Lang-4 按需触发（黑板瓶颈时）。**Lang-9 A5 深度内联 P1 ✅ P2 ✅ P3 ✅ P4 ✅ 全部完成（P1 模块内 trivial inline：IN01-IN08。P2 模块内一般内联：多语句/分支/循环/多 return exit label/用户函数嵌套/struct 参数/void/cleanup 守护/FO6 窗口修正/tempBaseline，IN09-IN23。P3 跨模块内联：ModuleInlineInfo + TryInlineMemberCall + _xInlineVars 导出变量重定向 XLOAD_MVAR/XSTORE_MVAR，XIN01-XIN10。P4 深度链式内联：callee 内部函数递归内联 + TryInlineCalleeFunc + visited 防环，DIN01-DIN05，1517 测试总计）。** Lang-10 ✅ 完成（导出变量默认值 + 批量提取 API，SK15 需求驱动）。Lang-11 ✅ 完成（模块级 struct var/const 直接初始化，MSV01-MSV15 全通过）。Lang-12 ✅ 完成（@export const 基础类型，EC01-EC03 全通过）。**Lang-13 ✅ 完成**（语法糖枚举，EN01-EN12 + LSP-EN01~EN05，1627 测试总计）。**Lang-14 ✅ 位运算支持完成**。**Lang-15 ✅ Include 可见性完成**（public/private 修饰符，PV01-PV18，1704 测试总计）。**Lang-16 ✅ Override 关键字完成**（显式跨文件替换，OV01-OV16，1734 测试总计）。**Lang-17 ✅ Include As 别名完成**（`include "path" as Alias` 命名空间隔离，IA01-IA12 全通过，1757 测试总计）。**Lang-18 ✅ Override Alias 声明完成**（`override func Alias.Name()` 替换别名模块声明，OA01-OA10 全通过，1777 测试总计）。**DX4-P0 ✅ LSP workspace 快速改善完成**（rootUri 解析 → 磁盘 FileSystemFileResolver + .ffvm.d.json AllDirectories 自动发现 + entryFunc=null 诊断专用模式 + include 指令 LSP 内解析，DX4-P0-01~13 全通过，1791 测试総計）。**DX4-P1 ✅ .ffproj 項目描述文件完成**（ProjectFile 解析类 + CompositeFileResolver 多路径 include + LSP 自動発見 .ffproj + includePaths/hostDeclarations/entry/compileOptions 全字段支持 + LoadProjectHostDeclarations，DX4-P1-01~16 全通过，1834 測試総計）。**DX4-P2 ✅ CLI 项目编译集成完成**（`ffvm-cli init` 脚手架 + `--host` 预设 + `compile/run --project` 读取 .ffproj 构建编译参数 + 共享 ProjectFile.GenerateTemplate/LoadHostDeclarations/MergeDeclarationJson，DX4-P2-01~13 全通过，1873 测试总计）。**DX4-P3 ✅ 跨文件符号查询完成**（合并 AST 缓存 _mergedAsts via Preprocessor + FilePathToUri OriginFile→URI 映射 + ResolveOriginUri 跨文件/同文件判定 + HandleDefinition/HandleReferences/HandleHover/HandleCompletion/HandleSignatureHelp 全部切换到合并 AST，DX4-P3-01~13 全通过，1901 测试总计）。**DX4-P4 ✅ LSP 辅助创建 .ffproj 完成**（`initialized` 触发检查有 .ffs 无 .ffproj → `window/showMessageRequest` 三选一 → Create → `workspace/applyEdit` CreateFile + TextDocumentEdit + GenerateTemplate 模板 + 服务端请求基础设施 SendRequest/_pendingServerRequests/HandleServerResponse，DX4-P4-01~13 全通过，1915 测试总计）。DX4 系列全部完成。**DX5 ✅ LSP 重命名 + 语义染色 + Include 导航 + 成员符号完成**（`textDocument/rename` + `prepareRename` 函数/变量/结构体/枚举重命名 + `textDocument/semanticTokens/full` 声明处语义染色 + Include 路径 go-to-definition/find-references + StructField/EnumMember definition/references + `.ffproj` 文件夹名自动创建 + `GenerateTemplate` 带注释模板 + `StripLineComments` + TextMate grammar 增强，DX5-01~20 全通过，1953 测试总计）。**DX6 ✅ Include 文件重命名自动更新引用完成**（LSP `workspace/willRenameFiles` 请求处理 + 普通 include / `include as` / 跨 includePaths / 子目录 / 显式 .ffs 扩展名覆盖 + capability 注册 + VSCode middleware，DX6-01~10 全通过，1975 测试总计）。**DX7 ✅ AST 精确位置追踪完成**（FieldAccessExpr.FieldNameLine/Column + VarDeclStmt/ParamDecl/StructField.TypeNameLine/Column + rename/references/semantic tokens 全覆盖，DX7-01~08 全通过，1984 测试总计）。**DX8 ✅ external func 语法 + 跨文件错误 + 表达式语义染色完成**（DX8-01~08 全通过，2071 测试总计）。**DX9 ✅ 语义染色改进完成**（DX9-01~08 全通过，2104 测试总计）。**E003 ✅ 紧急修复**（枚举类型/成员 findReferences 不完整 + didClose 缺失，E003-01~06 全通过，2113 测试总计）。**R1 ✅ LSP 架构重构完成**（AstWalker 基类统一 13 组遍历三元组 + ResolveSymbolDualAst 统一双 AST 符号解析 + DocumentStore 封装三字典，4100→3780 行，−8%，2113 测试全通过）。**E004 ✅ 模块级符号导航完成**（FindSymbolAtPosition 扫描 ModuleVariables + FindSymbolInExpr null func 守护 + FindDefinitionLocation 模块变量 + fallback + CollectReferencesWithOrigin 模块变量初始化器扫描 + CollectEnumIdentRefs 枚举类型引用，E004-01~10 全通过，2130 测试总计）。**DX10 ✅ 依赖图 + 全项目诊断完成**（Include 依赖图双向追踪 + OverlayFileResolver 内存内容可见 + RecompileDependents 级联传播 + workspace/didChangeWatchedFiles 磁盘变更感知，DX10-01~08 全通过，2127 测试总计）。**DX11 ✅ VFS + Rename 状态更新完成**（DocumentStore.RenameUri URI key 迁移 + ApplyTextEdits 内存文本编辑 + ApplyRenameState willRenameFiles 后状态同步 → 修复连续重命名失败，DX11-01~05 全通过，2140 测试总计）。**Lang/DX 系列全部完成。** 当前位置 → C 区间（宿主集成侧，待宿主 ECS 就绪）。DX12 ⚪ 远期待激活（[D_LspArchitecture.md](Discussion/D_LspArchitecture.md)）。 → [Step_Lang9](Plan/Step_Lang9_DeepInlining.md), [Step_Lang13](Plan/Step_Lang13_Enum.md), [Step_Lang14](Plan/Step_Lang14_Bitwise.md), [Step_Lang15](Plan/Step_Lang15_PublicPrivateVisibility.md), [Step_Lang17](Plan/Step_Lang17_IncludeAs.md), [Step_Lang18](Plan/Step_Lang18_OverrideAlias.md)
>
...
> - [x] A1 自动 getter 退化检测（AST 分析：0 params + 单 return + 单 module var 读取 → DegradationType.Getter + DegradeMvarSlot）
> - [x] A2 自动 setter 退化检测（AST 分析：1 param + 单 AssignExpr(moduleVar = param) → DegradationType.Setter + DegradeMvarSlot）
> - [x] VMConfig 新增 MaxXCallDepth / XCallDepthPolicy(Warn/Unlimited) 配置字段，VMWorld 构造函数接受 VMConfig
> - [x] 测试套件：AD01-AD12（getter/setter/non-pure/multi-stmt/literal/mixed/VMConfig Warn/Unlimited/custom depth/getter-with-param/setter-wrong-value）
...
> - [x] 全部 1627 测试通过（1574 原有 + 53 新增枚举测试），无回归
>
> **Lang-14 ✅ 位运算支持**：Lexer 6 新 TokenType（Amp/Pipe/Caret/Tilde/LtLt/GtGt），`&`/`|` 从 Error 回退改为合法单字符 token，`<`/`>` 扩展为 3 路判定。AST 6 新 NodeKind（BitAnd/BitOr/BitXor/BitNot/Shl/Shr）。Parser 插入 4 新优先级层：`|` < `^` < `&`（LogicalAnd 和 Equality 之间）、`<<`/`>>`（Comparison 和 Addition 之间）、`~`（一元与 `!`/`-` 同级）。Number 新增 6 个位运算静态方法（Fix64 + dev 双模式，整数域操作 ToInt → bitop → FromInt）。编译器 6 新 OpCode（BIT_AND=57/BIT_OR=58/BIT_XOR=59/BIT_NOT=60/SHL=61/SHR=62）+ BinOpCode/UnOpCode 映射 + TryFoldConstant 常量折叠 + GetRegisterMask 注册 + IsResultProducer Peephole 适配。VM 解释循环 6 新 case。TreeWalker 6 新 case。内联自动支持确认（BinaryExpr/UnaryExpr 在 IsInlineSafeExpr 中无条件安全，EstimateBodySize 按节点类型计数，新 NodeKind 无需改动）。性能无回归（Reg() 无改动，ExecuteInstance switch jump table 仅填空位）。BW01-BW15 编译器测试（46 asserts：基础运算、优先级、逻辑 vs 位运算、常量折叠、枚举+位运算 flag 模式、移位往返、XOR 交换、条件中位运算、函数参数、循环位操作、内联验证）+ TW-BW01~BW02 TreeWalker 测试（2 asserts）。1675 tests total (114 TW + 1153 Compiler + 44 Perf + 18 FFS + 51 Debug + 97 DAP + 198 LSP)。B01-B06 benchmark 无回归。
> - [x] 全部 1704 测试通过（1675 原有 + 29 新增 PV 可见性测试），B01-B06 无回归
>
> **R1 子计划 checklist**（LSP 架构重构 — DX10 前置清理）：
> - [x] Phase A: AstWalker 基类 — `WalkBlock`/`WalkStmt`/`WalkExpr` 单点 Stmt/Expr 分发 + `_abort` 字段支持搜索早退
> - [x] Phase A: 5 组引用收集三元组 → AstWalker 子类（CallRefsWalker, IdentRefsWalker, TypeRefsWalker, FieldAccessRefsWalker, EnumMemberAccessRefsWalker）
> - [x] Phase A: FindSymbolInBlock/Stmt/Expr → FindSymbolWalker（abort 早退）
> - [x] Phase A: FindHoverInBlock/Stmt/Expr → FindHoverWalker（abort 早退）
> - [x] Phase A: 3 组语义染色三元组 → AstWalker 子类（TypeUsageTokensWalker, VarNameTokensWalker, ExprSemanticTokensWalker）
> - [x] Phase A: FindVarType, FindVarStructType, CollectVariablesInScope → AstWalker 子类
> - [x] Phase B: `ResolveSymbolDualAst(uri, astLine, astCol)` → `(perFileTarget, resolvedTarget, perFileAst, mergedAst)` 统一双 AST fallback 模式
> - [x] Phase B: HandleDefinition / HandleReferences / HandlePrepareRename / HandleRename 4 个 handler 切换到统一方法
> - [x] Phase C: `DocumentStore` 内部类 — `SetContent`/`SetAst`/`SetMergedAst`/`GetAst`/`GetMergedAst`/`Remove`
> - [x] Phase C: 全部 16 处 `_documents`/`_documentAsts`/`_mergedAsts` 直接字典访问替换为 DocumentStore 调用
> - [x] 全部 2113 测试通过（114 TW + 1302 Compiler + 44 Perf + 18 FFS + 51 Debug + 97 DAP + 487 LSP），无回归
>
> **E004 子计划 checklist**（模块级符号导航 — 紧急修复）：
> - [x] FindSymbolAtPosition 扩展：扫描 ModuleVariables 列表（变量名位置匹配 + 类型注解位置匹配 + 初始化器表达式递归 FindSymbolInExpr）
> - [x] FindSymbolInExpr null func 守护：模块级上下文无 FuncDecl，参数搜索安全跳过
> - [x] FindDefinitionLocation 扩展：模块变量名 → 直接返回位置；函数作用域未找到 → fallback 到模块变量声明
> - [x] CollectReferencesWithOrigin 扩展：模块变量初始化器中 func/enum/enumMember 引用收集
> - [x] CollectEnumIdentRefsInBlock/Stmt/Expr：FieldAccessExpr 中枚举类型标识符引用收集
> - [x] E004-01~10 测试用例（17 asserts）：模块变量 go-to-def / find-references / hover / rename 全场景覆盖
> - [x] 全部 2130 测试通过（114 TW + 1302 Compiler + 44 Perf + 18 FFS + 51 Debug + 97 DAP + 504 LSP），无回归
>
...
**DIST-1~DIST-3 ✅ 已完成。DIST-8~DIST-9 ✅ 已完成。DIST-10 ✅ 已完成。D 阶段分发基础设施全部完成。Lang-1 ✅ 完成。Lang-1.1a ✅ 完成。Lang-1.1b ✅ 完成。Lang-2 ✅ 完成。Lang-3 ✅ 完成。**
...
| 全局 / 跨步骤 | H1 Handle64、PR1 带参配对、FIX1、DM1 双轨、B1 Editor DAP | BB1 黑板 Key ✅（Lang-3） |
| 部署架构 (MI) | MI-1 DAP 多实例、MI-2 SpawnScript、MI-3 KillInstance、MI-5 事件总线 | — |
| 数据读取 | MI-4 VMInspector | — |
| Cleanup / using | — | C5 超时 ✅、C6 合并 ✅ |
| 结构体 | MSV-F1 const 折叠（暂缓）、MSV-F2 @export struct（暂缓） | S4 参数 ✅、SN1 嵌套 ✅、SN2 字面量 ✅ |
| 脚本调试 | — | DBG1-DBG7 全部 ✅ |
| 语言服务 | — | LSP1-LSP7 全部 ✅ |
| Include As | IA-F1 LSP（暂缓）、IA-F3 变量读写（暂缓不推荐） | Lang-17 ✅、Lang-18 Override Alias ✅ |
| Include 可见性 | VIS-1 默认 private（暂缓，`include as` 占比 >50% 时评估）、VIS-2 LSP 诊断（暂缓） | Lang-15 ✅ |
| 易用性 (DX4~DX7) | **DX4-P0 LSP workspace ✅**、**DX4-P1 .ffproj ✅**、**DX4-P2 CLI 项目编译 + init 脚手架 ✅**、**DX4-P3 跨文件符号 ✅**、**DX4-P4 LSP 辅助创建 .ffproj ✅**、**DX5 LSP 重命名 + 语义染色 + Include 导航 + 成员符号 ✅**、**DX6 Include 文件重命名 ✅**、**DX7 AST 精确位置追踪 ✅** | — |
...
| 🟠 深远 | E003 | LSP 引用查找不完整 + didClose 缺失 — 枚举类型/成员 findReferences 不收集使用处 + 文档关闭无缓存清理 | ✅ 已修复 | [D_LspArchitecture.md](Discussion/D_LspArchitecture.md) |
| 🟠 深远 | E004 | 模块级符号导航缺失 — 模块变量初始化器/类型注解处 go-to-definition/find-references/hover/rename 不工作 | ✅ 已修复 | [D_LspArchitecture.md](Discussion/D_LspArchitecture.md) |
