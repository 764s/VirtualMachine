...
| *DX4-P0* | *LSP workspace 快速改善* | *⏳* | *LSP rootUri → 磁盘 IFileResolver 构建 + 自动发现 workspace 内 `.ffvm.d.json` 加载 + entryFunc=null 跳过入口检查。效果：单 workspace 内 include 路径解析 + syscall 补全基本可用。讨论来源 → [D_Usability_ModuleScope.md](Discussion/D_Usability_ModuleScope.md)* | *LSP 多文件易用性* | *LSP Server* | *⭐⭐* |
