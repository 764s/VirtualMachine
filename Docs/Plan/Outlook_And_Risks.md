...
| ID | 内容 | 触发时机 | 复杂度 |
|----|------|----------|--------|
| **STR1** | 常量字符串（日志打印 + 宿主传递，不支持拼接） | 需要日志输出或向宿主传递文本时 | 低（Lexer StringLiteral + VMProgram.StringConstants ROM + LOAD_CONST 索引；无新 OpCode，无快照影响，无 GC） |
...
### 2.10 脚本语言易用性改进（UC 系列）

> 来源：[D_ScriptUsabilityChecklist.md](../Discussion/D_ScriptUsabilityChecklist.md)（D18）
>
> 资深脚本语言使用者视角的系统性易用性审查（35 项检验标准 × 7 大类），
> 33/35 通过，2 项部分通过（E5 未使用变量警告、G5 字符串操作提示），0 项不通过。
> 以下为审查产出的改进建议，按优先级排列。

| ID | 内容 | 优先级 | 触发时机 | 复杂度 | 说明 |
|----|------|--------|----------|--------|------|
| **UC-1** | 未使用变量/参数警告 | 🔴 高 | 下一个 DX 窗口期 | ⭐⭐ | 编译器/LSP 诊断 `unused variable` / `unused parameter` warning。`_` 前缀约定标记有意不使用。可复用 F4 寄存器生命周期读写追踪 |
| **UC-2** | `break` / `continue` 语句 | 🔴 高 | 下一个 Lang 窗口期 | ⭐⭐ | 当前用 flag 变量模拟（如 KOF98 `hitDone`），降低可读性。需处理循环出口跳转 + Cleanup 栈展开 |
| **UC-3** | 块注释 `/* ... */` | 🟡 中 | 使用反馈驱动 | ⭐ | Lexer 层支持，不影响运行时 |
| **UC-4** | 复合赋值 `+=` `-=` `*=` `/=` | 🟡 中 | 使用反馈驱动 | ⭐ | 纯 Parser 语法糖，展开为 `lhs = lhs op rhs`，不需要新 OpCode |
| **UC-5** | 字符串操作专用编译期错误 | 🟡 中 | 使用反馈驱动 | ⭐ | 对 string 类型的 `+` 运算发出专用错误"不支持运行时拼接，请使用 Syscall"，减少迁移用户困惑 |
| **UC-6** | 定长数组 `var arr: int[8]` | 🟢 低 | 远期（业务需求出现时） | ⭐⭐⭐ | 编译期拍平为连续寄存器段（与 struct 拍平一致），零 GC 零违规。Parser + 编译器 + 运行时寄存器布局 |
| **UC-7** | Range-for 语法糖 `for i in 0..10` | 🟢 低 | 远期 | ⭐⭐ | Parser 层展开为 C 风格三段式 + FORLOOP 识别适配 |

**实施建议**：UC-1 + UC-2 为日常编写高频痛点，建议在下一个 DX/Lang 窗口期优先排入串行计划。UC-3~UC-5 按使用反馈激活。UC-6/UC-7 为远期展望。
...
| **3** | **O8** | 指令压缩 16B → 4B | L1 缓存 10-20% 加速 | 高 | ✅ B-η 完成（O8-1~O8-3/O8-5，O8-4 ⏸） |
...
**推荐顺序**：O1 ✅ → O2 ✅ → O6 ✅ → O15 ✅ → P0 ✅ → P1 ✅ → P2 ✅ → P3 ✅ → **O8 ✅ B-η 完成**。B3 Tier 1 详情见 [Step_B3_Optimization_Tier1.md](Step_B3_Optimization_Tier1.md)。

> **B-ε 串行优化计划已全部完成** — B-ε1~ε4（4/4 ✅）。
> 累计收益：B01-B06 平均 ~43% 时间优化，指令数平均减少 ~4 条。
> 详见 [Step_B_Epsilon_Perf.md](Step_B_Epsilon_Perf.md)。

**分支场景优化串行计划**（降低 B04 15x → 8x 目标，已纳入 §7 串行计划 B-ζ1~ζ3）：
...
| 优先级 | ID | 内容 | 预期收益 | 复杂度 |
|--------|----|------|---------|--------|
| ✅ 完成 | **FO1** | 叶函数优化（CALL_LEAF/RET_LEAF 跳过 CallFrame push/pop） | 叶函数开销 -40~60% | 低 | [B-β2](Step_B_Beta2_FO1_LeafFunction.md) |
...
| ID | 风险 | 影响 | 建议应对 |
|----|------|------|---------|
| **R5** | ~~结构体作为函数参数时寄存器窗口空间不足~~ | ✅ B-γ5 已解决：≤16 scratch regs 编译期限制 | S4 已与 FO6 联合评估完成 |
...
| **SR3** | ~~struct 变量与函数调用寄存器窗口交互~~ | ~~局部 struct 窗口偏移~~ | ✅ B-γ5 (S4) 已实施：struct 参数通过 scratch zone 正确传递，窗口隔离完好 |
...
| **GR3** | ~~文档缺口 D1-D4 未合并入总结文档~~ → ✅ 已合并：D1→§1.2、D2→§9.2、D3→§7.0、D4→§7.0b | ~~设计上下文缺失~~ | ✅ B-γ8 完成 |
...
| GR2 | ✅ 已修复 | **极低** | 不变 |
| GR3 | ~~⚠️~~ | **极低** | ✅ D1-D4 已合并入 VM_Summary.md（B-γ8 完成） |
...
> ✅ **B-γ8 已完成**：D1-D4 全部合并入 VM_Summary.md。

| # | 来源 | 内容 | 合并位置 |
|---|------|------|---------|
| D1 | VMScript.md | "条件→目标→数据效果→视觉效果"技能流水线模式 | ✅ §1.2「技能效果器流水线模式」 |
| D2 | VMScript2.md | 历史失败教训表 → 设计约束推导 | ✅ §9.2「历史失败教训与约束推导」 |
| D3 | VMScript4.md | 项目级成功标准（5 类验收维度） | ✅ §7.0「成功标准与验收维度」 |
| D4 | VMScript4.md | 设计验证递进轴线 | ✅ §7.0b「设计验证递进轴线」 |
...
| **DX17** | 统一符号解析 | ⏳ | 合并 SymbolAtPosition + FindDefinitionLocation 返回值为 ResolvedSymbol。HandleDefinition/HandleReferences/HandleRename/HandleHover 共享 ResolveSymbol 调用。消除 ResolveSymbolDualAst 二次查找。修复 AstWalker WaitForStmt 子表达式遗漏。讨论来源 → [D_LspStructuralAudit.md](../Discussion/D_LspStructuralAudit.md) §五 P1 + §八 | DX16 ✅ |
| **DX18** | 统一引用收集 | ⏳ | 7 个引用 Walker（CallRefs/IdentRefs/TypeRefs/StructLiteralTypeRefs/EnumIdentRefs/FieldAccessRefs/EnumMemberAccessRefs）→ 1 个 UnifiedRefsWalker。CollectReferencesWithOrigin 8 个 SymbolKindTag 分支 → 声明位置 + 统一遍历入口。消除 CollectReferences/CollectReferencesWithOrigin 二重性。652 现有 LSP 测试提供回归保障。讨论来源 → [D_LspStructuralAudit.md](../Discussion/D_LspStructuralAudit.md) §五 P2+P3 | DX17 ✅ |

> Lang-18 ✅ 完成。DX4-P0 ✅ 完成。DX4-P1 ✅ 完成。DX4-P2 ✅ 完成。DX4-P3 ✅ 完成。DX4-P4 ✅ 完成。DX5 ✅ 完成。DX6 ✅ 完成。DX7 ✅ 完成。DX8 ✅ 完成。DX9 ✅ 完成。E003 ✅ 完成。R1 ✅ 完成。E004 ✅ 完成。DX10 ✅ 完成。DX11 ✅ 完成。DX13 ✅ 完成。DX14 ✅ 完成。DX15 ✅ 完成。DX16 ✅ 完成。2278 测试总计（652 LSP）。DX12 ⚪ 远期待激活。DX17 ⏳ 统一符号解析。DX18 ⏳ 统一引用收集。当前位置 → DX17。
...
| UC-1 | 未使用变量/参数警告 | 下一个 DX 窗口期（日常编写高频痛点） |
| UC-2 | `break` / `continue` 语句 | 下一个 Lang 窗口期（循环可读性痛点） |
| UC-3 | 块注释 `/* ... */` | 使用反馈驱动 |
| UC-4 | 复合赋值 `+=` `-=` `*=` `/=` | 使用反馈驱动 |
| UC-5 | 字符串操作专用编译期错误 | 使用反馈驱动 |
| UC-6 | 定长数组 | 远期（业务需求出现时） |
| UC-7 | Range-for 语法糖 | 远期 |
...
| **R5** | ✅ ≤16 scratch regs 编译报错 — B-γ5 已实施 | B-γ5 (S4) ✅ | 与 FO6 联合评估完成，编译期安全限制就位 |
