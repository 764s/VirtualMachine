...
### 2.10 寄存器扩展 — 模块变量区（来源：Lang-1 讨论）

> 背景：Lang-1 模块变量占用 r56~r63（8 slots，绝对寻址）。当前 KOF98 典型需求 1~3 个 var slot（真正可变的跨函数状态），
> `const` 编译期折叠不占寄存器。8 slots 对当前及可预见需求完全足够。
> 以下三个方案按"临时快速增加 / 正式增加 / 兜底"分层备案，均暂不排期。

| ID | 方案 | 内容 | 改动量 | 性能影响 | 触发时机 |
|----|------|------|--------|---------|----------|
| **E1** | 压缩 temp zone（临时快速） | TempRegBase 48→52，module zone 从 8 扩到 12 slots。FO6 自适应重映射保证 temp 窗口缩小不影响功能 | ⭐ 极低（改 1 常量） | **零** — temp 压缩由编译器 FO6 重映射吸收 | module var 超 8 个时 |
| **E2** | 扩大 MaxRegisters（正式） | MaxRegisters 64→96 或 128。module zone 可扩到 32~64 slots | ⭐⭐ 低（改常量 + NumberRegisters 固定数组扩容 + 快照结构体调整） | 每实例 RAM +256B~+512B，快照拷贝量等比增加 | E1 仍不够时 |
| **E3** | 溢出到辅助存储（兜底） | module var 超限时溢出到常量池/堆，通过 load/store 指令间接访问 | ⭐⭐⭐ 高（新 OpCode + 编译器溢出路径 + VM 运行时间接访存） | 热路径从寄存器读变为间接访存，**有性能下降** | E1+E2 均不足时（极端场景） |
