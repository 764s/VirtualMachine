...
| KL-03 | ~~struct字面量名不计入struct重命名~~ | ~~低~~ | ✅ DX14 已修复。struct 字面量中的类型名现在计入 rename 编辑 |
| KL-04 | ~~private 不过滤跨文件补全~~ | ~~中~~ | ✅ DX15 已修复。`private func/struct/enum/var` 不再出现在 include 文件的 completion 列表中 |
| KL-05 | ~~同名变量引用不隔离作用域~~ | ~~低~~ | ✅ DX16 已修复。同名 `var x` 的 references 现在按作用域精确匹配，内外层变量隔离 |

所有 5 个已知限制（KL-01~05）均已修复。
...
| ~~DX12-12 struct 重命名阈值 ≥2~~ | ~~struct 字面量名 `Vec2 { ... }` 未被 rename 覆盖（KL-03）~~ | ✅ DX14 已修复。断言升级为 ≥3（struct 字面量名现已计入） |
| ~~DX12-22 同名变量未断言隔离~~ | ~~内外层同名 `var x` 的 references 都返回全部同名引用（KL-05）~~ | ✅ DX16 已修复。断言升级为 `==3` / `==2`（作用域隔离已生效） |
| ~~DX12-23 private 过滤走"已知限制"路径~~ | ~~`private func` 仍出现在跨文件补全中（KL-04）~~ | ✅ DX15 已修复。断言升级为严格 `!hasSecret`（private 符号过滤已生效） |

**共同模式**：发现了真实问题，但本次任务为"测试审查"非"Bug 修复"，故调整断言为当前行为下限，同时标注偏差根因。修复需求已提交串行计划 → DX13~DX16。**所有 5 个 KL 均已修复完成。**
...
| DX15 | KL-04 | ✅ Private 跨文件补全过滤（HandleCompletion IsPrivate + IsFromOtherFile 守卫） | ⭐⭐ |
| DX16 | KL-05 | ✅ 变量引用作用域隔离（FindSymbolWalker 块级 _activeDecls + ScopedIdentRefsWalker + FindDefinitionLocation declLine 直达） | ⭐⭐⭐ |
