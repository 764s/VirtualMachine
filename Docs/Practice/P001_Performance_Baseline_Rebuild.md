...
> **结论**：本实践提出的 3 组建议已全部分流处理。性能基线重建（4.1）与基准设计改进（4.3）合并为展望条目 BM1；
> 执行循环优化（4.2）纳入展望条目 O15。所有改动须经串行计划正式步骤推进。详见各条目的「**处理结果**」标注。
...
> **处理结果**：🔵 **回收至展望计划** — 新增展望条目 [BM1](../Plan/Outlook_And_Risks.md#28-benchmark-基础设施来源p001)（Benchmark 基础设施改进），
> 涵盖环境感知 Δ 计算、基线环境说明、Warmup/Scale 调参、CI 自我基线等 5 项建议。Benchmark 驱动，待业务需要时排入串行计划。
...
> **处理结果**：🔵 **回收至展望计划** — 新增展望条目 [O15](../Plan/Outlook_And_Risks.md#32-调整型优化--解释器热路径benchmark-驱动)（ExecuteInstance 热循环优化），
> 涵盖哨兵指令方案、AggressiveOptimization JIT 提示、MaxStepsPerTick 局部缓存。本地验证 VM 执行时间下降 31-65%，待 Benchmark 驱动排入串行计划。
...
> **处理结果**：🔵 **回收至展望计划** — 纳入展望条目 [BM1](../Plan/Outlook_And_Risks.md#28-benchmark-基础设施来源p001)（Benchmark 基础设施改进），
...
| ID | 建议 | 类型 | 优先级 | 处理结果 |
|----|------|------|--------|---------|
| 4.1 | 性能基线重建（环境感知 Δ + Warmup/Scale 调参 + CI 自我基线） | CI / 基准设施 | 中 | 🔵 回收至展望 → BM1 |
| 4.2 | 执行循环优化（哨兵指令 + AggressiveOptimization + 局部缓存） | 运行时优化 | 中 | 🔵 回收至展望 → O15 |
| 4.3 | 基准设计改进（B05 说明 + B06 新增） | 基准改进 | 低 | 🔵 回收至展望 → BM1 |
