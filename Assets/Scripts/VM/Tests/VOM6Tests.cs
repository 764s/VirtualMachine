...
/// <summary>
/// VOM6 validation: <see cref="VMEngine.Batch"/> row-batched dispatch.
/// </summary>
public static partial class VOM6Tests
{
    ...
    public static void RunAll()
    {
        _passed = 0;
        _failed = 0;
        TestHarness.BeginSuite("VOM6Tests");

        RunBasicTests();
        RunValidationTests();
        RunPerfTests();

        Debug.Log($"[VOM6Tests] {_passed} passed, {_failed} failed");
        TestHarness.EndSuite();
        if (_failed > 0)
            throw new Exception($"VOM6Tests failed: {_failed} assertion(s)");
    }
}
