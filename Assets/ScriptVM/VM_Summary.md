...
9. 结构体编译期拍平验证  ✅ 303 项 Assert 通过
  ...
  F4 + 自然优化 + 调试基础设施                         ⏳
      ...
      脚本调试 · 第一阶段（编译器侧，随 F4 合并，边际排期极低）：
        DBG1. 源码映射表（Source Map）
        DBG2. 符号表（Symbol Table）
      ↓
  ┌────────────────────────────────────────────────────────┐
  │  V5: 帧内 Profiler 验证  ← 真实 Syscall 接入 ECS 后   │
  │  C4 强制检查 ✅ 已就位                                │
  ...
  ┌────────────────────────────────────────────────────────┐
  │  脚本调试 · 第二阶段（运行时调试能力）        ⏳      │
  │    DBG3. 断点机制（Breakpoint）                       │
  │    DBG5. 变量查看（Variable Inspection）               │
  │    DBG6. 调用栈查看（Call Stack Inspection）           │
  │  可在 Step 10 编辑器之前以测试/命令行方式验证         │
  └────────────────────────────────────────────────────────┘
      ↓
  ┌────────────────────────────────────────────────────────┐
  │  Handle64 批处理协议（展望项）                        │
...
10. 编辑器流程图投影
  ...
  │  Tier 3+: O8, O9-O14, FO1-FO3, FO6, SO1              │
  │  按 benchmark 数据排序，逐项验证收益后推进             │
  └────────────────────────────────────────────────────────┘
      ↓
  ┌────────────────────────────────────────────────────────┐
  │  脚本调试 · 第三阶段（编辑器调试 UI）         ⏳      │
  │    DBG4. 单步执行（Step Over / Into / Out）            │
  │    DBG8. 调试协议（Debug Protocol）                    │
  │    DBG7. 编辑器调试 UI                                │
  │  依赖 Step 10 编辑器基础框架                          │
  └────────────────────────────────────────────────────────┘
      ↓
