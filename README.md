...
FFVM 是一个**寄存器式字节码虚拟机**，为 2D 格斗/动作游戏中的**技能、子弹、状态效果**等战斗逻辑而设计。
配套的脚本语言 **FFScript**（`.ffs`）语法类似 Go，提供帧级挂起（`wait` / `yield`）和自动清理（`defer` / `using`）作为一等语义，让技能脚本可以像写过程式代码一样表达复杂的多阶段战斗流程。
...
在自研的 2D 横版动作游戏中，战斗逻辑分散在技能系统、子弹系统、状态效果系统三套独立体系里。
三者各自运行良好，但整体缺乏统一的执行模型——挂起 / 等待 / 生命周期边界等控制语义没有成为一等公民，快照 / 回滚也无法被统一到同一种执行模型中。

FFVM 的目标是：**用一套统一的脚本 VM 把这三类战斗逻辑收拢到脚本层**，并满足以下硬约束：

| 约束 | 说明 |
|------|------|
| **零 GC** | 战斗中零运行时垃圾回收 |
| **memcpy 快照 / 回滚** | 全部运行状态为定长值类型，`Array.Copy` 即可完成帧同步回滚 |
| **`wait` 一等语义** | 脚本挂起不依赖宿主协程或行为树 `Running` |
| **ROM / RAM 分离** | 字节码只读共享，实例状态为纯值 ECS 组件 |
| **Fix64 确定性数值** | 开发期 float 快速迭代，发布期 Fix64 确保帧同步一致 |
...
**未完成**：真实 Syscall 接入 ECS（C1）、帧内 Profiler 验证（C2）、帧同步集成验证（C5）、编辑器流程图投影（C6）。
...
```
  ...
  ⚪ 真实 Syscall 接入 ECS      — 连接游戏世界（等待宿主就绪）
